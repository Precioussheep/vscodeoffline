...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--artifacts", dest="artifactdir", default="../artifacts/", help="Path to downloaded artifacts"
    )
    parser.add_argument(
        "--frequency",
        dest="frequency",
    ...
    parser.add_argument(
        "--check-binaries", dest="checkbinaries", action="store_true", help="Check for updated binaries"
    )
    parser.add_argument(
        "--check-insider", dest="checkinsider", action="store_true", help="Check for updated insider binaries"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--update-extensions", dest="updateextensions", action="store_true", help="Download extensions"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--vscode-version", dest="version", default="1.69.2", help="VSCode version to search extensions as."
    )
    parser.add_argument(
        "--total-recommended",
        type=int,
    ...
    if config.logfile:
        ...
        logzero.logfile(log_file_loc, maxBytes=1000000, backupCount=3)

    # convert namespace into something actually useful by a linter
    config_cls: VSCSyncConfig = VSCSyncConfig.from_dict(vars(config))

    if config_cls.artifactdir and config_cls.updatebinaries and not config_cls.artifactdir.is_dir():
    ...
    while True:
        ...
        if not config_cls.frequency:
            ...
            log.info(f"Going to sleep for {utils.seconds_to_human_time(config_cls.frequency)}")
            time.sleep(config_cls.frequency)
