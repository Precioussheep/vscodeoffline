...
On the Internet connected system , __vscsync__ will:
...
On the non-Internet connected system, __vscgallery__:
...
2. Sort out SSL/TLS within your environment to support offline use.
    ...
    * Deploy the bundled root and intermediate certificate authority (vscoffline/vscgallery/ssl/ca.crt and ia.crt), with the obvious security tradeoff.
    ...
    __Windows 10__: Import the certificates into the machine's trusted root certificate authority (Start > "Manage Computer Certificates").

    __Darwin__: Import the certificates into the machine's trusted root certificate authority.

    __Ubuntu__: Easiest method seems to be Open Chrome, navigate to
